...
its_build_id, = struct.unpack("<I", port.read(4))[0]
...
cur_state = STATES[ord(self.port.read(1))]
...
def generate_raspberry_pi(states, build_id):
    #STATES = [State(name, id, {deviceName : DeviceState(OrderedDict([(name, WireValue(type,changed,value)...]), [eventName...]))...}...] 
    new_states = []
    for i, (name, state) in enumerate(states.items()):
        #list comp from hell
        new_states.append(State(name,i,{devName : DeviceState(OrderedDict((val, WireValue(ty,False,None)) for val,ty in device.values.iteritems()), [e for e in device.events]) for (devName,device) in state.iteritems()}))
    return PI_SOURCE_TEMPLATE.format(states=new_states, build_id=build_id)
...
if __name__ == '__main__':
    ...
    debug = generate_debug(states, build_id)
    pi = generate_raspberry_pi(states,build_id)
    open(os.path.join(dirname, "debug.py"), 'wb').write(debug)
    open(os.path.join(dirname, "Manager.py"), 'wb').write(pi)
