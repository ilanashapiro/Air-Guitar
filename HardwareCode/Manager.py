    ...
    def __init__(self,*args):
        if len(args) == 3:
            ...
            if not connected:
                ...
                sys.exit(2)
            self.build_id = 3290321593
            self.hardware_states = [State(name='IDLE', id=0, devices={'master': DeviceState(values=OrderedDict(), events=[]), 'tablet': DeviceState(values=OrderedDict(), events=[])}), State(name='PISTON', id=1, devices={'master': DeviceState(values=OrderedDict([('encodedChord', WireValue(type='uint32_t', changed=False, value=None))]), events=['playFChord', 'playBbChord', 'playD5Chord', 'playAbChord', 'playDbChord', 'playCChord', 'playeChord', 'playaChord', 'playDChord', 'playGChord', 'playAChord', 'playC_InvertedChord', 'playF_InvertedChord', 'retractAll', 'engageMuteBar', 'disengageMuteBar']), 'tablet': DeviceState(values=OrderedDict(), events=[])})]
